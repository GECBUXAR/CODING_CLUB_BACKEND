  ...
  try {
    ...
    const result = await Result.findById(resultId)
      ...
      .populate({
        ...
        model: "Question",
        select:
          "questionText questionType options correctAnswer correctOption points",
      });
    ...
    if (
      !isAdmin &&
      result.user._id.toString() !== requestingUserId.toString()
    ) {
    ...
    const showDetails =
      isAdmin ||
      result.event.examDetails?.showResultsImmediately ||
...
export const getExamStatistics = asyncHandler(async (req, res) => {
  try {
    ...
    for (const exam of exams) {
      ...
      const passRate =
        totalParticipants > 0 ? (passedCount / totalParticipants) * 100 : 0;
      ...
      results.forEach((result) => {
        ...
        lowestScore =
          totalParticipants > 0
            ? Math.min(lowestScore, result.percentageScore)
            : 0;
...
/**
 * Get results for a specific event/exam
 * Access: Admin only
 */
export const getResultsByEventId = asyncHandler(async (req, res) => {
  try {
    ...
    // Only allow admins to view all results for an event
    if (!isAdmin) {
      throw new ApiError(
        403,
        "Only administrators can access all event results"
      );
    }
    ...
    throw new ApiError(
      error.statusCode || 500,
      error.message || "Error retrieving event results"
    );
  }
});
...
export const getUserPerformance = asyncHandler(async (req, res) => {
  try {
    ...
    if (!isAdmin && requestingUserId.toString() !== userId) {
      throw new ApiError(
        403,
        "You are not authorized to view this performance data"
      );
    }
    ...
    results.forEach((result) => {
      ...
      worstScore =
        totalExams > 0 ? Math.min(worstScore, result.percentageScore) : 0;
    });
    ...
    const performanceTrend = Object.values(monthlyPerformance).sort((a, b) =>
